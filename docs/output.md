...
└── plugins
    ...
    ├── mccs_stack_preprocessing.cppipe
...
* `imctools/<SAMPLE>/<ROI>/mccs_stack/*.tiff`  
* `imctools/<SAMPLE>/<ROI>/mccs_stack.ome.tiff`  

*Description*:  

* Parse data files for ROI information and convert each channel into individual `tiff` files.
* Uses `metadata.csv` to sort `tiff` files into corresponding folders i.e. `full_stack/` or `mccs_stack/`.
...
* `preprocess/<SAMPLE>/<ROI>/mccs_stack/composite.tiff`  
...
```bash
Caused by: Missing output file(s) 'mccs_stack/*' expected by process 'PreprocessIlastikStack (file_name_roi_1)'
```
